...
def write_spinsys_multi_half(
    ...
    if use_ms:
        ...
        for i, ms in enumerate(msiso):
            block += ("{{{0} {1} {2} " "{3} {4} {5} 1}} \n").format(
                   ms,  msaniso[i], msasymm[i], *eulangs[i]
            )
    global out_file
...
SimpsonTemplatesHalf = namedtuple(
    "SimpsonTemplates",
    """BASIC_SPECTRUM,
                              BROADENED_SPECTRUM
                              """,
...
SimpsonTemplatesHalf = SimpsonTemplatesHalf(
    BASIC_SPECTRUM={
        "pars": {},
        "pulseq": """
   global par
...
def simpson_xy_multi_half(filename,newfile_name):
...
class SimpsonSequenceMultiHalf:
    ...
    def __init__(self, spinsys_source):
        ...
        self.apply_template(SimpsonTemplatesHalf.BASIC_SPECTRUM)
    ...
    def write_input_multi_half(self, path=None, LB = '100', ZeroFill = '8192'):
...
#        outf += "\tset par(etaQ)     [lindex $Spin_pars 1]\n"
        outf += "\tset par(CSiso)    [lindex $Spin_pars 0]\n"
        outf += "\tset par(CSaniso)  [lindex $Spin_pars 1]\n"
        outf += "\tset par(CSeta)    [lindex $Spin_pars 2]\n"
        outf += "\tset par(alpha)    [lindex $Spin_pars 3]\n"
        outf += "\tset par(beta)     [lindex $Spin_pars 4]\n"
        outf += "\tset par(gamma)    [lindex $Spin_pars 5]\n"
        outf += "\tset par(weight)   [lindex $Spin_pars 6]\n"
#        outf += "\tset par(ppp) \"p\"\n"
